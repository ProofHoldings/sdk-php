...
use ProofHoldings\Resources\Authorizations;
use ProofHoldings\Resources\Hitl;
...
class ResourcesTest extends TestCase
{
    ...
    public function testVerificationRequestsGetProofsGets(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/verification-requests/vr_123/proofs')
            ->willReturn(['proofs' => [['type' => 'phone', 'identifier' => '+37069199199', 'channel' => 'whatsapp', 'proof_token' => 'jwt_proof']]]);

        $vr = new VerificationRequests($http);
        $result = $vr->getProofs('vr_123');
        $this->assertCount(1, $result['proofs']);
        $this->assertEquals('jwt_proof', $result['proofs'][0]['proof_token']);
        $this->assertEquals('+37069199199', $result['proofs'][0]['identifier']);
    }
    ...
    private function authorizationData(array $overrides = []): array
    {
        return array_merge([
            'id' => 'auth_123',
            'status' => 'active',
    ...
    private function createAuthorizationResponseData(array $overrides = []): array
    {
        return array_merge([
            'id' => 'auth_123',
            'status' => 'pending',
    ...
    public function testAuthorizationsCreatePostsToEndpoint(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('post')
            ->with('/api/v1/authorizations', [
                'phone' => '+15551234567',
                'channel' => 'whatsapp',
                'business_name' => 'Acme Corp',
            ])
            ->willReturn($this->createAuthorizationResponseData());

        $a = new Authorizations($http);
        $result = $a->create([
            'phone' => '+15551234567',
            'channel' => 'whatsapp',
            'business_name' => 'Acme Corp',
        ...
        $this->assertInstanceOf(CreateAuthorizationResponse::class, $result);
        $this->assertEquals('auth_123', $result->id);
    }

    public function testAuthorizationsRetrieveGetsByIdAndEncodes(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/authorizations/auth%2Fspecial%26id')
            ->willReturn($this->authorizationData(['id' => 'auth/special&id']));

        $a = new Authorizations($http);
        $result = $a->retrieve('auth/special&id');

        $this->assertInstanceOf(Authorization::class, $result);
        $this->assertEquals('auth/special&id', $result->id);
    }

    public function testAuthorizationsListPassesFilters(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/authorizations', ['status' => 'active', 'channel' => 'whatsapp'])
            ->willReturn([
        ...
        $a = new Authorizations($http);
        $result = $a->list(['status' => 'active', 'channel' => 'whatsapp']);

        $this->assertInstanceOf(AuthorizationListResponse::class, $result);
    }

    public function testAuthorizationsRevokeDeletesWithReason(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('delete')
            ->with('/api/v1/authorizations/auth_123', ['reason' => 'No longer needed'])
            ->willReturn($this->authorizationData(['status' => 'revoked']));

        $a = new Authorizations($http);
        $result = $a->revoke('auth_123', ['reason' => 'No longer needed']);

        $this->assertInstanceOf(Authorization::class, $result);
        $this->assertEquals('revoked', $result->status);
    }

    public function testAuthorizationsExportPassesParams(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/authorizations/export', ['format' => 'json', 'status' => 'active'])
            ->willReturn(['data' => [], 'total' => 0]);

        $a = new Authorizations($http);
        $result = $a->export(['format' => 'json', 'status' => 'active']);
    ...
    public function testConfirmationsCreatePosts(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('post')
            ->with('/api/v1/confirmations', [
                'action' => 'deploy:prod',
                'message' => 'Deploy v2.3?',
                'hitl_id' => 'hitl_123',
            ])
        ...
        $c = new \ProofHoldings\Resources\Confirmations($http);
        $result = $c->create([
            'action' => 'deploy:prod',
            'message' => 'Deploy v2.3?',
            'hitl_id' => 'hitl_123',
        ]);
        $this->assertInstanceOf(\ProofHoldings\Types\Confirmation::class, $result);
        $this->assertEquals('conf_123', $result->id);
        $this->assertEquals('pending', $result->status);
    }

    public function testConfirmationsRetrieveGets(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/confirmations/' . rawurlencode('conf_123'))
            ->willReturn([
                'id' => 'conf_123',
                'hitl_id' => 'hitl_1',
                'action' => 'deploy:prod',
        ...
        $c = new \ProofHoldings\Resources\Confirmations($http);
        $result = $c->retrieve('conf_123');
        $this->assertInstanceOf(\ProofHoldings\Types\Confirmation::class, $result);
        $this->assertEquals('approved', $result->status);
    }

    public function testConfirmationsListGets(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/confirmations', ['status' => 'pending', 'hitl_id' => 'hitl_1'])
            ->willReturn([
                'data' => [],
        ...
        $c = new \ProofHoldings\Resources\Confirmations($http);
        $result = $c->list(['status' => 'pending', 'hitl_id' => 'hitl_1']);
        $this->assertInstanceOf(\ProofHoldings\Types\ConfirmationListResponse::class, $result);
    }
    ...
    private function hitlData(array $overrides = []): array
    {
        return array_merge([
            'id' => 'hitl_123',
            'user_id' => 'user_1',
            'name' => 'Production Approvals',
            'channels' => [['type' => 'telegram', 'config' => ['chat_id' => '123456']]],
            'timeout_seconds' => 1800,
            'status' => 'active',
            'created_at' => '2025-01-01T00:00:00Z',
            'updated_at' => '2025-01-01T00:00:00Z',
    ...
    public function testHitlCreatePosts(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('post')
            ->with('/api/v1/hitl', ['name' => 'Production Approvals', 'channel' => 'telegram'])
            ->willReturn($this->hitlData());

        $h = new Hitl($http);
        $result = $h->create(['name' => 'Production Approvals', 'channel' => 'telegram']);
        $this->assertInstanceOf(HitlType::class, $result);
        $this->assertEquals('hitl_123', $result->id);
    }

    public function testHitlRetrieveGets(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/hitl/hitl_123')
            ->willReturn($this->hitlData());

        $h = new Hitl($http);
        $result = $h->retrieve('hitl_123');
        $this->assertInstanceOf(HitlType::class, $result);
        $this->assertEquals('hitl_123', $result->id);
    }

    public function testHitlRetrieveEncodesId(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/hitl/hitl%2Fspecial')
            ->willReturn($this->hitlData(['id' => 'hitl/special']));

        $h = new Hitl($http);
        $h->retrieve('hitl/special');
    }

    public function testHitlListGets(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('get')
            ->with('/api/v1/hitl', ['status' => 'active'])
            ->willReturn([
                'data' => [$this->hitlData()],
                'pagination' => ['page' => 1, 'limit' => 20, 'total' => 1, 'total_pages' => 1],
            ]);
        ...
        $h = new Hitl($http);
        $result = $h->list(['status' => 'active']);
        $this->assertInstanceOf(HitlListResponse::class, $result);
    }

    public function testHitlUpdatePatches(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('patch')
            ->with('/api/v1/hitl/hitl_123', ['name' => 'Updated'])
            ->willReturn($this->hitlData(['name' => 'Updated']));

        $h = new Hitl($http);
        $result = $h->update('hitl_123', ['name' => 'Updated']);
        $this->assertInstanceOf(HitlType::class, $result);
        $this->assertEquals('hitl_123', $result->id);
    }

    public function testHitlDeleteDeletes(): void
    {
        $http = $this->mockHttp();
        $http->expects($this->once())
            ->method('delete')
            ->with('/api/v1/hitl/hitl_123')
            ->willReturn($this->hitlData(['status' => 'archived']));

        $h = new Hitl($http);
        $result = $h->delete('hitl_123');
