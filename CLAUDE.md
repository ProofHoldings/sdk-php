...
- `src/Resources/*.php` — 25 resource classes (Verifications, Proofs, Sessions, Authorizations, Confirmations, etc.)
...
- `composer.json` includes a `classmap` entry for `src/Types.php` because it defines 118 DTO classes in a single file, which PSR-4 cannot resolve (PSR-4 requires one class per file)
...
- `Proofs` resource caches JWKS keys for offline verification (`verifyOffline()`), with `refreshJwks()` to force cache invalidation after key rotation
- `HttpClient::delete()` accepts optional body parameter for DELETE-with-body requests (used by authorizations revoke)
